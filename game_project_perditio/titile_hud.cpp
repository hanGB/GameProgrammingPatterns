...
TitleHud::TitleHud()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		if (m_menu[i]) m_menu[i]->Init(PERVec2(-0.8, -0.2 - 0.2 * i), PERVec2(0.5, 0.15), PERColor(200, 200, 200), PERColor(150, 150, 150),
			true, 1, PERColor(0, 0, 0), false, 1, PERColor(0, 0, 0),
			true, text[i], 0.1, PERVec2(-0.79, -0.2 - 0.2 * i), PERColor(0, 0, 0), PERColor(50, 50, 50));
		else PERLog::Logger().ErrorWithFormat("%s 텍스트가 생성되지 않았습니다.", text[i]);
	}
}
